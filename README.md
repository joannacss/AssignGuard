...
- `reassign_papers.py`: recommends replacement reviewers for the conflicted reviewers reported by find_assignments_coi.py. It keeps the highest-preference reviewer in each conflict group, matches the replacement to the removed reviewer's assignment type, always ranks candidates primarily by TPMS score, and applies the configured workload cap only to regular reviewers.
...
3. Count each reviewer's total assignment workload and separately count each AreaChair's `Main_MR` assignment workload.
...
7. If the removed reviewer is `Main` or `Main_AR`, only consider replacement candidates whose PC `tags` include `RegRev` and whose total assignment workload is below `--max-workload`.
8. Rank eligible regular-reviewer candidates by highest TPMS score. Ties prefer lower total workload, then email.
9. If the removed reviewer is `Main_MR`, only consider replacement candidates marked as `AreaChair` in PC `roles` or `tags`. The `--max-workload` threshold does not apply to these candidates.
10. Rank eligible AreaChairs by highest TPMS score, then the fewest current `Main_MR` assignments, then email. AreaChair workload only breaks TPMS ties and never overrides a better TPMS match.
11. Exclude candidates already assigned to the same paper for both assignment types.
12. After making a recommendation, increment the candidate's relevant workload so later recommendations in the same run account for the new assignment.
...
- `--max-workload` (default: `14`): maximum total assignment workload for regular-reviewer candidates. This threshold does not apply to AreaChairs replacing `Main_MR` reviewers.
...
Each recommendation includes the reviewer to replace, that reviewer's current workload, and the recommended new reviewer with TPMS score and current workload before assignment. For regular-reviewer slots, `current_workload` is the total number of assigned papers. For `Main_MR` slots, it is the number of current `Main_MR` assignments.
